...
Une seule commande, à lancer telle quelle : elle résout le chemin du script puis l'exécute.
Remplace uniquement le JSON par les trois services retenus.
...
echo '{"courses":[{"name":"Poireaux vinaigrette","price":12},{"name":"Quasi de veau","price":26},{"name":"Paris-brest","price":9}]}' | python3 "$SCRIPT"
```

Ne lance **pas** de commande de repérage séparée avant celle-ci : la première ligne suffit quand
`${CLAUDE_PLUGIN_ROOT}` est défini, la deuxième ne se déclenche que sinon. Le répertoire courant
du shell n'étant pas celui de la skill, un chemin relatif comme `scripts/price_range.py`
échouerait. N'explore jamais la racine du système (`find / ...`).
